...
function getPrice(val) {
  return typeof val === 'object' ? val.price : val
}

function getBestUrl(product) {
  const sorted = Object.entries(product.prices).sort((a, b) => getPrice(a[1]) - getPrice(b[1]))
  const bestBrand = sorted[0][0]
  if (product.prices[bestBrand]?.url) return product.prices[bestBrand].url
  return BRAND_URLS[bestBrand]?.(product.name) || `https://www.google.com/search?q=${encodeURIComponent(product.name)}&tbm=shop`
}
...
function ProductCard({ product, selectedBrands }) {
  ...
  return (
    <div style={{ background:'#16162A', border:'1px solid #2A2A45', borderRadius:16, padding:20, position:'relative', display:'flex', flexDirection:'column' }}>
      {product.popular && (
        <span style={{ position:'absolute', top:12, right:12, background:'rgba(124,58,237,0.15)', color:'#8B5CF6', border:'1px solid rgba(124,58,237,0.35)', fontSize:9, fontWeight:700, padding:'2px 8px', borderRadius:20 }}>POPULAIRE</span>
      )}
      ...
      {saving > 0.01 && (
        <div style={{ marginTop:10, padding:'7px 11px', background:'rgba(16,185,129,0.08)', borderRadius:9, border:'1px solid rgba(16,185,129,0.2)', fontSize:12, color:'#10B981' }}>
          Économie potentielle : <strong>{saving.toFixed(2)} €</strong> par {product.unit}
        </div>
      )}

      {product.tip && (
        <div style={{ marginTop:8, padding:'7px 11px', background:'rgba(124,58,237,0.06)', borderRadius:9, border:'1px solid rgba(124,58,237,0.15)', fontSize:11, color:'#A89FC0', lineHeight:1.5 }}>
          💡 {product.tip}
        </div>
      )}

      <a href={getBestUrl(product)} target="_blank" rel="noopener noreferrer sponsored" style={{ display:'block', marginTop:12, padding:'10px 0', borderRadius:10, background:'#7C3AED', color:'#fff', fontSize:13, fontWeight:500, textAlign:'center', textDecoration:'none' }}>
    ...
    <div style={{ maxWidth:1280, margin:'0 auto', padding:'40px 24px 80px' }}>
      ...
      <div style={{ background:'#16162A', border:'1px solid #2A2A45', borderRadius:20, padding:24, marginBottom:24 }}>
        ...
        <div style={{ display:'flex', gap:7, flexWrap:'wrap', marginBottom:16 }}>
          {cats.map(c => (
            <button key={c} onClick={() => setCat(c)} style={{ padding:'5px 13px', borderRadius:20, border:`1px solid ${cat===c ? '#7C3AED' : '#2A2A45'}`, background: cat===c ? 'rgba(124,58,237,0.2)' : 'none', color: cat===c ? '#8B5CF6' : '#6B6585', fontSize:12 }}>{c}</button>
          ))}
        </div>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}>
          ...
          {Object.entries(BRANDS).map(([brand, b]) => (
            <button key={brand} onClick={() => toggleBrand(brand)} style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 12px', borderRadius:20, border:`1px solid ${selectedBrands.includes(brand) ? b.color : '#2A2A45'}`, background: selectedBrands.includes(brand) ? `${b.color}20` : 'none', color: selectedBrands.includes(brand) ? b.color : '#6B6585', fontSize:12 }}>
      ...
      <div style={{ marginBottom:20, color:'#6B6585', fontSize:13 }}>
        <span style={{ color:'#F5F3FF', fontWeight:500 }}>{filtered.length}</span> produits · Cliquez sur une enseigne pour accéder au prix exact
      </div>
